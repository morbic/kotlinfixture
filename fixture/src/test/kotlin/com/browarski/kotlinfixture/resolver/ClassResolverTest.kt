...
import com.browarski.kotlinfixture.FixtureTestJavaClass
...
import com.browarski.kotlinfixture.decorator.constructor.GreedyConstructorStrategy
import com.browarski.kotlinfixture.decorator.constructor.ModestConstructorStrategy
...
class ClassResolverTest {
    ...
    fun `Can override Java constructor arg`() {
        repeat(100) {
            val context = context.copy(
                configuration = ConfigurationBuilder().apply {
                    property<com.browarski.kotlinfixture.FixtureTestJavaClass, String>("arg0") { "custom" }
                }.build()
            )
    ...
    fun `Can override Java setter`() {
        repeat(100) {
            val context = context.copy(
                configuration = ConfigurationBuilder().apply {
                    property<String>(com.browarski.kotlinfixture.FixtureTestJavaClass::setMutable) { "custom" }
                }.build()
            )
